...
public class nHelper {
    ...
    public static HashMap<String, Double> stateValues;
    ...
    public static void play(String argv[]) throws Exception {
        ...
        //     stateValues = (HashMap<List<Integer>, Double>) ois.readObject();
        //     ois.close();
        //     fis.close();
        // } else {
        ...
        File stateValueFile = new File("StateValues.dat");

        if (stateValueFile.exists() && stateValueFile.length() > 0){
            fis = new FileInputStream(stateValueFile);
            ois = new ObjectInputStream(fis);
            stateValues = (HashMap<String, Double>) ois.readObject();
            ois.close();
            fis.close();
        } else stateValues = new HashMap<>();

        File p1File = new File("p1Move.txt");
        if (p1File.exists()) p1File.delete();
        File p2File = new File("p2Move.txt");
        if (p2File.exists()) p2File.delete();

        BufferedWriter p1writer;
        BufferedWriter p2writer;
        ...
        if (player1 == 1) {
            ...
            nPlayer.FindBestMove(1,board,bestmove);
            ...
            //write move tooken board state to file
            p1writer = new BufferedWriter(new FileWriter(p1File, true));

            p1writer.write(convolve(board));
            ...
            // writeDataToFile(boardToString(board), "p2Move.txt");

            nPlayer.FindBestMove(2,board,bestmove);
            ...
            //write move tooken board state to file
            // writeDataToFile(boardToString(board), "p1Move.txt");
        ...
        for (;;) {
            ...
            p2writer = new BufferedWriter(new FileWriter(p2File, true));

            p2writer.write(convolve(board));
            ...
            if (bestmove[0] != 0) { /* There is a legal move */
                ...
                // writeDataToFile(convolve(board), "visited.txt");
    ...
    //              {
    //                  if(nHelper.color(board[y][x])==2)  boardString.append("-1.3,");
    //                  else  boardString.append("1.3,");
    //              }
    //              else if(nHelper.piece(board[y][x]))
    //              {
    //                  if(nHelper.color(board[y][x])==2) boardString.append("-1,"); 
    //                  else  boardString.append("1,");
    //            }
    //         }
    //     }
    ...
    public static String convolve(char[][] board){
        ...
        for (i=0;i<5;i++){
            last[i] = conv2[i][0] + conv2[i][1] + conv2[i+1][0]+conv2[i+1][1];
        }
        ...
        for (i = 0; i<5; i++){
            // for (int j = 0; j < 2; j++){
                if (last[i] > 0) boardString.append("1,");
                else if (last[i] < 0) boardString.append("-1,");
                else  boardString.append("0,");
            // }
        }
        boardString.deleteCharAt(boardString.length() - 1);
        boardString.append("\n");
        
        // List<Integer> stateRep = new ArrayList<>();
        // for (i=0;i<5;i++){
        //     if (last[i] > 0) stateRep.add(1);
        //     else if (last[i] < 0) stateRep.add(-1);
        //     else stateRep.add(0);
        // }
    ...
    // public static double[] boardToDoubleArray(char[][] board){

    //     double[] alsoBoard = new double[32];
    //     int k = 0;
    ...
    //              else if(nHelper.king(board[y][x]))
    //              {
    //                  if(nHelper.color(board[y][x])==2)  alsoBoard[k++] = 1.3;
    //                  else  alsoBoard[k++] = -1.3;
    //              }
    //              else if(nHelper.piece(board[y][x]))
    //              {
    //                  if(nHelper.color(board[y][x])==2) alsoBoard[k++] = 1;
    //                  else  alsoBoard[k++] = -1;
    //            }
    //         }
    //     }
    ...
    public static double getPrediction(State state){
        String thisBoard = convolve(state.board);
        Double predict = stateValues.get(thisBoard);
        if (predict== null) return 0.5;
        return predict;
    }
}
