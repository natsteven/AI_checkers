...
public class nPlayer {
    ...
    public static void FindBestMove(int player, char[][] board, char[] bestmove) {
        ...
        //     // System.err.println("Eval of board: " + evalBoard(nextState));
            // double temp = min(nextState, moveVal, Double.POSITIVE_INFINITY, 6);
            double temp = (1-nHelper.getPrediction(nextState));
            if(temp > moveVal){
                moveVal = temp;
                myBestMoveIndex = x;
            }
    ...
    static double min(State state, double alpha, double beta, int depth){
        if (depth<=0){
            double score = nHelper.getPrediction(state);
            if (state.player==1) return score;
            else return 1-score;
        } 
        depth--;
    ...
    static double max(State state, double alpha, double beta, int depth){
        if (depth<=0){
            double score = nHelper.getPrediction(state);
            if (state.player==2) return score;
            else return 1-score;
        } 
        depth--;
    ...
    // {
    //     // List<Integer> stateRep = nHelper.convolve(state.board);
