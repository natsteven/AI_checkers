...
public class JENPlayerHelper {
    ...
    static int FindLegalMoves(JENState state) {
        int x, y;
    ...
    public static void play(String argv[]) throws Exception {
        ...
        if (player1 == 1) {
            ...
            JENPlayer.FindBestMove(1,board,bestmove, end);
            ...
            JENPlayer.FindBestMove(2,board,bestmove, end);
            if (bestmove[0] != 0) { /* There is a legal move */
                mlen = MoveLength(bestmove);
                PerformMove(board, bestmove, mlen);
                buf = MoveToText(bestmove);
